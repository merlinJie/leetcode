...
class Solution {
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (int i = 0; i < cpdomains.length; i++) {
            String[] tag = cpdomains[i].split(" ");
            Integer iTag = Integer.parseInt(tag[0]);
            String[] dos = tag[1].split("\\.");
            String temp = tag[1];
            for(int j = 0; j < dos.length; j++) {
                if(j != dos.length) {
                    if(map.containsKey(temp)) {
                        map.put(temp, iTag + map.get(temp));
                    }else {
                        map.put(temp, iTag);
                    }
                    temp = temp.replace(dos[j] + ".", "");
                } else {
                    if(map.containsKey(dos[j])) {
                        map.put(dos[j], iTag + map.get(dos[j]));
                    }else {
                        map.put(dos[j], iTag);
                    }
                };
